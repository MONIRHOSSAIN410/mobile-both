...
echo ==== fix-backend-git START ==== > "%LOG%" 2>&1

where git >nul 2>&1
if errorlevel 1 set "PATH=%PATH%;C:\Program Files\Git\cmd"
git --version >> "%LOG%" 2>&1
if errorlevel 1 (
  echo RESULT: FAIL-GIT-NOT-FOUND >> "%LOG%" 2>&1
  goto :end
)
...
if exist "backend\.git\" (
  if exist "D:\myproject-backend-git-backup\" (
    echo RESULT: ABORT-BACKUP-FOLDER-ALREADY-EXISTS >> "%LOG%" 2>&1
    goto :end
  )
  move "backend\.git" "D:\myproject-backend-git-backup" >> "%LOG%" 2>&1
  if errorlevel 1 (
    echo RESULT: FAIL-COULD-NOT-MOVE-NESTED-GIT >> "%LOG%" 2>&1
    goto :end
  )
  echo moved backend\.git to D:\myproject-backend-git-backup >> "%LOG%" 2>&1
) else (
...
findstr /i /c:"env.NEW" /c:".env" "D:\myproject\staged-files.txt" >nul 2>&1
if not errorlevel 1 (
...
:verify
echo. >> "%LOG%" 2>&1
echo --- STEP 7: verify backend/server.js is in HEAD --- >> "%LOG%" 2>&1
git ls-tree -r --name-only HEAD backend >> "%LOG%" 2>&1

:end
echo. >> "%LOG%" 2>&1
echo ==== fix-backend-git DONE ==== >> "%LOG%" 2>&1
exit
